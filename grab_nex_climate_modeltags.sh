...
# you can run this script and pipe list into a file with 'bash grab_nex_climate_modeltags.sh | see file.txt'

grep '^mkdir' make_directories_files.sh | grep '/historical/' | sed -e 's:^.*/historical/::' | sed -e 's:/pr/.*::' | sed -e 's:/tasmin/.*::' | sed -e 's:/tasmax/.*::' | uniq > tags.csv
