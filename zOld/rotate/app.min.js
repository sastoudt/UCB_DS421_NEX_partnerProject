!function() {
    ...
    function t(t, n, e) {
        ...
        t.append("path").datum(d3.geo.graticule()).attr("class", "graticule").attr("d", n), t.append("path").datum({
            ...
            }).attr("class", "point").attr("d", o).style("fill","black"));
    ...
    d3.selectAll("#map, #eisenlohr").data([n(a, o).scale(245).translate([a / 2, .56 * o]), d3.geo.eisenlohr().precision(.1).clipExtent([[-1, -1], [a + 1, o + 1]]).translate([a / 2, o / 2]).scale(75).rotate([0, -30])]).append("svg").attr("width", a).attr("height", o).each(function(n) {
        ...
        function a(e, a, u) {
            ...
            var m = d3.behavior.drag().on("drag", function() {
                ...
                g.text(e + "=" + l(a.angle) + "\xb0"), d3.select("#rotations").selectAll("svg").each(function(t) {
     ...
     function(t, n) {
        ...
        d3.selectAll("svg").insert("path", ".foreground").datum(topojson.feature(n, n.objects.land))
        .attr("class", "land").style("fill","silver"), d3.selectAll("svg").insert("path", ".foreground").
        datum(topojson.mesh(n, n.objects.countries)).attr("class", "mesh"),
		...
		/* .data(data).enter()*/
		 ...
		 coordinates: [-122.2727,37.8716]
   		 ...
   		 .style("fill", "mediumblue").style("stroke-width", "2px")
   		 /*.attr("d",path)*/		 
...
/* tried to add a json for the oceans but can't quite figure out where it should go */

/* d3.json("oceans.json")
        function(t, n) {
        d3.selectAll("svg").insert("path", ".foreground").datum(topojson.feature(n, n.objects.Ocean))
        .attr("class", "land").style("fill","whitesmoke") */ 
